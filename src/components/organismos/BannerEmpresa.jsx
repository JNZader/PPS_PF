...
export function BannerEmpresa() {
  ...
  if (!dataempresa?.nombre) {
    return <div>Loading empresa data...</div>;
  }
    ...
    <Container>
      <div className="content-wrapper-context">
        <span className="titulo">
          {<v.iconoempresa />}
          {dataempresa.nombre || 'No empresa data'}
        </span>
        <div className="content-text">
          FiveStock te mantiene siempre informado.
        </div>
        <ContentCards>
          <CardDatosEmpresa
            titulo="Moneda"
            valor={dataempresa.simbolomoneda || 'N/A'}
          />
          <CardDatosEmpresa titulo="Usuarios" valor={contadorusuarios || 0} />
        </ContentCards>
      </div>
